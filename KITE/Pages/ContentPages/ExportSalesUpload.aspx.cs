...
{
    public partial class ExportSalesUpload : System.Web.UI.Page
    {
        private List<ExportSalesWithKilosConvertionViewModel> CsvDataList;
        ...
        public void btnDownloadToCsv(object sender, EventArgs e)
        {
            ...
            string fileName = $"{DateTime.Now:yyyyMMdd}KITE_ExportSale.csv";
        ...
        {
            errorLabel.Text = "";
            ExportSalesFunctionModel exportSalesFunction = new ExportSalesFunctionModel();
            Tuple<Exception, string, List<ExportSalesWithKilosConvertionViewModel>> fileResult = exportSalesFunction.ExportSalesReadCsvFile(fileUpload);
            if (fileResult.Item1.Message != "null")
            {
                UtilityModel errorHandler = new UtilityModel();
                Exception loadCsvException = new Exception("Terdapat masalah ketika mensubmit file csv. Mohon untuk cek kembali apakah data file csv cocok dengan format upload ExportSales.<br/> Detail : " + fileResult.Item1.Message);
        ...
        private void ExportSalesBindGridView()
        {
            ViewState["Row"] = 0;
        ...
        private void LoadCsvData()
        {
            try
            {
                Tuple<object, Exception> readCsvResult = new ReadCsvModel().ReadCsvFunction("exportSales", (string)Session["FilePath"], ";", ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString);
                if (readCsvResult.Item2.Message != "null")
                {
                    UtilityModel errorHandler = new UtilityModel();
                    Exception loadCsvException = new Exception("Terdapat masalah ketika memuat file csv. Mohon untuk cek kembali apakah data file csv cocok dengan format upload ExportSales process.<br/> Detail : " + readCsvResult.Item2.Message);
                    errorHandler.UploadCsvErrorHandler(loadCsvException, new GridView[] { CsvDataGridView }, errorLabel);
                }
        ...
        protected void btnUpload_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Tuple<DataTable, Exception> checkFGperBatch = new DatabaseModel().SelectTableIntoDataTable("UUID", "FG_Tracing", $" WHERE Year_Period = '{yearPeriod}' AND Month_Period = '{monthPeriod}' ", ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString);
