...
{
    public partial class Input_KITED : System.Web.UI.Page
    {
        ...
        protected void SubmitInformation(string Company, string Plant, string Material_LOW, string Material_HIGH, string Sloc_LOW, string Sloc_HIGH, string MoveT_LOW, string MoveT_HIGH, string sDate_LOW, string sDate_HIGH, string User_IP)
        {
            ...
            KC.InsertQueueD(Company, Plant, Material_LOW, Material_HIGH, Sloc_LOW, Sloc_HIGH, MoveT_LOW, MoveT_HIGH, sDate_LOW, sDate_HIGH, HttpContext.Current.Session["UserAppID"].ToString(), "Open", "Open Report KITE D", HttpContext.Current.Session["UserAppID"].ToString(), HttpContext.Current.Session["FullName"].ToString());
        ...
        private void ListMove()
        {
            ...
            var dt = KC.Select_Move("REPORT_D");
        ...
        private void show()
        {
            ...
            ReportViewer1.ServerReport.ReportPath = "/ReportKITE/KITE_D"; // Report Name 
        ...
        {
            bool submit = false;
            var dt = KC.Select_CheckStatusPrev(HttpContext.Current.Session["UserAppID"].ToString(), "Open Report KITE D");
            //string status = "";
            if (dt.Count() > 0)
            {
                foreach (var select in dt)
