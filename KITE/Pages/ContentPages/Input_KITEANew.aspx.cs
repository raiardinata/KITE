...
{
    public partial class Input_KITEANew : System.Web.UI.Page
    {
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (check)
            {
                ...
                if (boleh)
                {
                    ...
                    strUserIp = localip;
                    //SubmitInformation();
                    SubmitInformation(strCompany, strPlant, strMaterialLow, strMaterialHigh, strDateLow, strDateHigh, strUserIp);
                }
            }
        }

        protected void SubmitInformation(string Company, string Plant, string Material_LOW, string Material_HIGH, string sDate_LOW, string sDate_HIGH, string User_IP)
        {

            KC.InsertQueueA(Company, Plant, Material_LOW, Material_HIGH, "","", sDate_LOW, sDate_HIGH, HttpContext.Current.Session["UserAppID"].ToString(), "Open", "Open Report KITE A", HttpContext.Current.Session["UserAppID"].ToString(), HttpContext.Current.Session["FullName"].ToString());
            ScriptManager.RegisterClientScriptBlock(this, typeof(string), "Close", "Close('#Insert')", true);
            ScriptManager.RegisterClientScriptBlock(this, typeof(string), "Open", "ShowPopUp('#Loading')", true);
            LabelWaitingList.Visible = false;
