...
namespace KITE.Controllers
{
    public class LoginController
    {
        ...
        public List<Models.CatchExeptions> InsertLog(string action, string explanation, string status, string userappid, string username)
        {
            ...
            List<Models.CatchExeptions> Result = dc.ExecuteQuery<Models.CatchExeptions>("Exec UserApp_Login_UserLOGACT @action={0} , @explanation={1} , @status={2}, @userappid={3}, @username={4}", param).ToList<Models.CatchExeptions>();
            return Result;
        }
