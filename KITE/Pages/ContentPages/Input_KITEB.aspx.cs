...
{
    public partial class Input_KITEB : System.Web.UI.Page
    {
        ...
        {
            bool submit = false;
            var dt = KC.Select_CheckStatusPrev(HttpContext.Current.Session["UserAppID"].ToString(), "Open Report KITE B");
            //string status = "";
            if (dt.Count() > 0)
            {
                foreach (var select in dt)
