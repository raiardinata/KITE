...
{
    public partial class GIRawMaterialUpload : System.Web.UI.Page
    {
        private List<GIRawMaterialWithConvertionViewModel> CsvDataList;
        ...
        public void btnDownloadToCsv(object sender, EventArgs e)
        {
            ...
            string fileName = $"{DateTime.Now:yyyyMMdd}KITE_GIRawMaterial.csv";
        ...
        {
            errorLabel.Text = "";
            GIRawMaterialFunctionModel giRawMaterialFunction = new GIRawMaterialFunctionModel();

            Tuple<Exception, string, List<GIRawMaterialWithConvertionViewModel>> fileResult = giRawMaterialFunction.GIRawMaterialReadCsvFile(fileUpload);
            if (fileResult.Item1.Message != "null")
            {
                UtilityModel errorHandler = new UtilityModel();
                Exception loadCsvException = new Exception("Terdapat masalah ketika mensubmit file csv. Mohon untuk cek kembali apakah data file csv cocok dengan format upload GI Raw Material.<br/> Detail : " + fileResult.Item1.Message);
        ...
        private void GIRawMaterialBindGridView()
        {
            ViewState["Row"] = 0;
        ...
        private void LoadCsvData()
        {
            try
            {
                Tuple<object, Exception> readCsvResult = new ReadCsvModel().ReadCsvFunction("giRawMaterial", (string)Session["FilePath"], ";", ConfigurationManager.ConnectionStrings["DefaultConnection"].ConnectionString);
